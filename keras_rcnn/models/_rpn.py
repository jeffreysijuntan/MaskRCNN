...
class RPN(keras.models.Model):
    ...
    def __init__(self, image, classes, feature_maps=None, features=256):
        ...
        for index, feature_map in enumerate(feature_maps):
            name = "p{index_substr}".format(index_substr = index + 2)
            ...
            target_anchors, target_scores, target_bounding_boxes = keras_rcnn.layers.AnchorTarget(base_size=(feature_map - 1), scales=[0.5, 1, 1.5])([metadata, bounding_boxes, scores])
